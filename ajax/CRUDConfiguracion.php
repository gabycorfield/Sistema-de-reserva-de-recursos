<?php  //CRUD tabla detallereserva-db escuela
    ...
    if(isset($_POST['accion_fechas'] )&& $_POST['accion_fechas']){ //accion_detalle toma distintos valores según lo que va a hacer
      ...
      if(isset($_POST['accion_conf'] )&& $_POST['accion_conf']){ //accion_detalle toma distintos valores según lo que va a hacer
        $accion = $_POST['accion_conf'];
       
        //muestra el listado de fechas no reservables 
        if ( $accion === "mostrar"){ 
            $sql = "SELECT minReserva, maxReserva, diasAnticipacion FROM config ";
            $result = mysqli_query($conn, $sql);
            $array = array();
            if (mysqli_num_rows($result) > 0) {
               // output data of each row
        ...
        if ( $accion === "cambiar1"){ 
          $hora=$_POST['hora'];
            $sql = "UPDATE config SET minReserva = '$hora' ";
            
            if (mysqli_query($conn, $sql)) {
              echo "Actualización exitosa";
            } else {
                echo "Error actualizando la hora: " . mysqli_error($conn);
            }
            mysqli_close($conn);
        }
        ...
        if ( $accion === "cambiar2"){ 
          $hora=$_POST['hora'];
            $sql = "UPDATE config SET maxReserva = '$hora' ";
        ...
        if ( $accion === "cambiar3"){ 
          $dato=$_POST['dias'];
            $sql = "UPDATE config SET diasAnticipacion = '$dato' ";
            
            if (mysqli_query($conn, $sql)) {
              echo "Actualización exitosa";
            } else {
                echo "Error actualizando los dias: " . mysqli_error($conn);
            }
            mysqli_close($conn);
        }
        ...
        if ( $accion === "cambiar4"){ 
          $dato=$_POST['dias'];
            $sql = "UPDATE config SET diasAmplitud = '$dato' ";
